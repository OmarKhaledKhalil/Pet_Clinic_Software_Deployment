...
resource "aws_security_group" "k8s_sg" {
  ...
  # Allow SSH from Jenkins Host (your office/jenkins public IP)
  ingress {
  ...
  # Allow SSH from within the subnet (for internal connections)
  ingress {
    from_port   = var.ssh_port
    to_port     = var.ssh_port
    protocol    = "tcp"
  ...
  # Kubernetes API from Jenkins/CI systems
  ingress {
